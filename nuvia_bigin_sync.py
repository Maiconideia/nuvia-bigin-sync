...
class SyncState:
    ...
    processed_messages: Dict[str, List[str]] = None
    error_count: int = 0
    sync_count: int = 0
...
class NuviaAPI:
    ...
    def search_contacts(self, phone: str) -> Optional[List[Dict]]:
        result = self._request('GET', '/contacts', params={'search': phone})
...
class BiginAPI:
    ...
    def find_contact(self, phone: str) -> Optional[str]:
        result = self._request('GET', '/Contacts', params={'criteria': f'(Phone:equals:{phone})'})
    ...
    def add_note(self, contact_id: str, note_text: str) -> Optional[str]:
        note_data = {'Note_Content': note_text, 'Contact': contact_id}
